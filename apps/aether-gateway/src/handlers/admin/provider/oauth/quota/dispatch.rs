...
use super::glm_coding_plan::refresh_glm_coding_plan_provider_quota_locally;
...
const PROVIDER_QUOTA_REFRESH_HANDLERS: &[(&str, ProviderQuotaRefreshHandler)] = &[
    ...
    (
        "glm_coding_plan",
        refresh_glm_coding_plan_provider_quota_locally_boxed,
    ),
...
fn refresh_glm_coding_plan_provider_quota_locally_boxed<'a>(
    state: &'a AdminAppState<'a>,
    provider: &'a StoredProviderCatalogProvider,
    endpoint: &'a StoredProviderCatalogEndpoint,
    keys: Vec<StoredProviderCatalogKey>,
    ...
    Box::pin(refresh_glm_coding_plan_provider_quota_locally(
        state,
        provider,
        endpoint,
        keys,
